...
class ParameterPanel(QFrame):
    ...
    def __init__(self):
        ...
        self.dask_workers_spinbox = QSpinBox()
        # Set a sensible range, from 1 to the number of CPU cores
        cpu_count = os.cpu_count() or 4 # Default to 4 if cpu_count is None
        self.dask_workers_spinbox.setRange(1, cpu_count)
        self.dask_workers_spinbox.setValue(cpu_count // 2) # Default to half the cores

        dask_layout.addWidget(dask_label)
        dask_layout.addStretch()
        dask_layout.addWidget(self.dask_workers_spinbox)
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        return {
            "steps": steps,
            "dask_workers": self.dask_workers_spinbox.value()
        }
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        if "dask_workers" in config:
            self.dask_workers_spinbox.setValue(config["dask_workers"])
